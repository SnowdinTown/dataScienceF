...
def getRecommendCase(type, offset):
	...
	enabled_cases = list(filter(lambda x: base + RANK_SCOPE < x['rank_score'] or x['rank_score'] < base - RANK_SCOPE, enabled_cases))
	
	res_cases = []
	
	while sum(map(lambda x:x['rank_score'], res_cases)) < GROUP_MAX_SCORE and len(res_cases) < GROUP_MAX_NUM and len(scoped_cases) > 0:
